...
export const CreateCompetition = async (body, token) => {
  ...
  try{
    const response = await axios.post(CREATE_COMPETITION_API, body, {"Content-Type": "multipart/form-data", headers:{Authorization: `Bearer ${token}`}})
...
export const UpdateCompetition = async (body, token) => {
  const toastId = toast.loading("Loading...")
  try{
    const response = await axios.post(UPDATE_COMPETITION_API, body, {"Content-Type": "multipart/form-data", headers:{Authorization: `Bearer ${token}`}})
...
export const DeleteCompetition = async (competitionId, token) => {
  const toastId = toast.loading("Loading...")
  try {
    const response = await axios.post(DELETE_COMPETITION_API, competitionId, {headers:{Authorization: `Bearer ${token}`}})
